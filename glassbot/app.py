...
Run with::
    ...
    streamlit run glassbot/app.py          # from workspace root
    streamlit run app.py                   # from inside glassbot/
"""

from __future__ import annotations

# ---------------------------------------------------------------------------
# sys.path fix — must happen before any glassbot imports.
#
# app.py lives inside the glassbot/ package directory.  When Streamlit runs it,
# the workspace root (parent of glassbot/) is NOT automatically on sys.path, so
# `import glassbot` fails with "No module named 'glassbot'".
#
# We detect the workspace root from __file__ and prepend it to sys.path.
# This works for all launch styles:
#   streamlit run glassbot/app.py          (from workspace root)
#   streamlit run app.py  (cwd=glassbot/)  (from inside the package)
# ---------------------------------------------------------------------------
import os
import sys

_APP_DIR = os.path.dirname(os.path.abspath(__file__))   # .../polaris-poc/glassbot
_WORKSPACE_ROOT = os.path.dirname(_APP_DIR)             # .../polaris-poc
if _WORKSPACE_ROOT not in sys.path:
    sys.path.insert(0, _WORKSPACE_ROOT)

# Load .env from the glassbot/ directory (where the .env file lives)
try:
...
try:
    from glassbot.config import Config
    from glassbot.exceptions import ConfigurationError
    from glassbot.chatbot.agent import build_agent, run_agent
    from glassbot.utils.helpers import rows_to_dataframe, format_execution_time
...
def main() -> None:
    ...
    if st.session_state.get("agent_error"):
        st.error(
            ...
            "**Setup instructions:**\n"
            "1. Copy `.env.example` to `.env` in the `glassbot/` directory.\n"
